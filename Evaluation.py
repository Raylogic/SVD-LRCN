...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for prediction, truth in zip(self.prediction, self.truth):
            if prediction == truth and prediction == 1:
                TP += 1
            elif prediction == truth and prediction == 0:
                TN += 1
            elif prediction != truth and prediction == 1:
                FP += 1
            elif prediction != truth and prediction == 0:
                FN += 1
            else:
    ...
    def metrics(self):
        ...
        prediction = precision_score(self.prediction, self.truth)
        ...
        prediction_neg = precision_score(self.prediction, self.truth)
        ...
        prediction = (prediction + prediction_neg) / 2
        recall = (recall + recall_neg) / 2
        f1measure = (f1measure + f1measure_neg) / 2

        # Return the metrics
        return accuracy, prediction, recall, f1measure
